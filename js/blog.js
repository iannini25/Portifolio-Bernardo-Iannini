...
function postCardHtml(post, layout, views) {
  ...
  return `
    <article class="blog-post-card ${layout} ${hasCover ? '' : 'blog-post-card--no-image'}"
             ...
             data-cover="${escapeHtml(post.cover || '')}">
      ${hasCover ? `
        <div class="blog-post-cover">
          <img src="${escapeHtml(post.cover)}" alt="${escapeHtml(post.coverAlt || post.title)}" loading="lazy" width="800" height="500"/>
        </div>
...
function renderIdentityFeatured() {
  ...
  target.dataset.cover = featured.cover || '';
  target.innerHTML = `
    <div class="bid-featured-inner">
      ${hasCover ? `
        <div class="bid-featured-img">
          <img src="${escapeHtml(featured.cover)}" alt="${escapeHtml(featured.title)}" loading="lazy"/>
        </div>
...
function setupCenterPreview() {
  ...
  const setPreview = (post) => {
    ...
    const apply = () => {
      currentPost = post;
      img.style.backgroundImage = `url("${post.cover}")`;
